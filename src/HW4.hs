...
instance (Jsonable a, Jsonable b, Jsonable c) => Jsonable (a, b, c) where
  ...
  fromJson (JsonArray [a, b, c]) = case (fromJson a, fromJson b, fromJson c) of
    (Just d, Just e, Just f) -> Just (d, e, f)
    _ -> Nothing
  fromJson _ = Nothing
...
instance (Num a, Eq a) => Semigroup (SparseMatrixSum a) where
  (<>) (SparseMatrixSum (SparseMatrix r1 c1 e1)) (SparseMatrixSum (SparseMatrix _ _ e2))
     = SparseMatrixSum (SparseMatrix r1 c1 entriesSum)
    where
      merged = Map.unionWith (+) e1 e2
      entriesSum = Map.filter (/= 0) merged

instance (Num a, Eq a) => Semigroup (SparseMatrixMult a) where
  (<>) (SparseMatrixMult (SparseMatrix r1 _ e1)) (SparseMatrixMult (SparseMatrix _ c2 e2)) = 
    SparseMatrixMult (SparseMatrix r1 c2 result)
    where
...
pathsOfLengthK _ _ _ (Matrix []) = 0
pathsOfLengthK 0 i j (Matrix _) = if i == j then 1 else 0
...
hasPath _ _ (Matrix []) = False
hasPath i j mat@(Matrix rows)
  | i == j = True
  | otherwise = foldr (||) False [ pathsOfLengthK k i j mat > 0
                                | k <- [1 .. length rows - 1]
                                ]
